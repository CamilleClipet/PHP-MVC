...
class ArticlesController extends AppController
{
  ...
  public function showArticles()
  {
    ...
    foreach($result as $data)
    {
      ...
      <a class="btn btn-dark btn-md" href=href="?url=ArticlesController/singleArticle/'.$data['article_id'].' " role="button">View more</a>
      </div>';
  ...
  public function showSingleArticle($id)
  {
    ...
    if ($comments != NULL)
    {
      ...
      if($_SESSION['group'] == 2 || $_SESSION['id'] == $data['id_author']){
        $article .= '<p><a href="?url=ArticlesController/deleteArticle/'.$data['article_id'].'" onclick="return confirm(\'Are you sure you want to delete this article?\');"><button type="button" class="btn btn-dark">Delete</button></a>

        <a href="?url=ArticlesController/editionArticle/'.$data['article_id'].'"><button type="button" class="btn btn-dark">Edit</button></a></br>
        </p>
        </div>';
      }
  ...
  public function search()
  {
    if($_POST != NULL)
    {
      ...
      foreach($result as $data)
      {
        ...
        //               </div>';
        $articles .= '<div class="jumbotron">
        <h1 class="display-8">'.$data['title'].'</h1>
        By : '.$data['username'].'</br>
        '.$date. '</br>
              <p></p>
        <p class="lead">'.$data['content'].'</p>
